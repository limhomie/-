...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (content) {
        content.innerHTML = '<p style="padding: 20px; color: red;">测试：profile.js 已执行</p>';
    }
});
// 模拟数据存储
 // 模拟动态数据
            ...
            <div class="post-images-detail">
                ${post.images.map((img, index) => `
                    <div class="image-container">
                        <img src="${img}" alt="动态图片 ${index + 1}" class="post-image-detail" 
...
function toggleLike(postId, likeBtn = null) {
    ...
    if (likeBtn) {
        ...
        // 更新详情页的按钮
        const detailLikeBtn = document.querySelector('.post-actions-detail .like-btn');
        if (detailLikeBtn) {
...
function updateLikeButton(button, isLiked, likesCount) {
    ...
    const countSpan = button.querySelector('.action-count') || button.querySelector('.like-count');
...
function loadComments(postId, container = null) {
    ...
    const targetContainer = container || document.getElementById(`comments-${postId}`);
    
    if (!targetContainer) return;
    
    if (postComments.length === 0) {
        targetContainer.innerHTML = '<div class="no-comments">还没有评论，快来抢沙发吧！</div>';
        return;
    }
    ...
    targetContainer.innerHTML = postComments.map(comment => `
        <div class="comment-item" data-comment-id="${comment.id}">
            ...
            <div class="comment-content">
                ...
                <div class="comment-actions">
                    <button class="reply-btn" onclick="replyToComment(${comment.id}, '${comment.username}')">
                        回复
                    </button>
...
function replyToComment(commentId, username) {
    ...
    const postId = commentElement.closest('.comments-section') ? 
        commentElement.closest('[data-post-id]').dataset.postId : null;
...
function initPostEditor() {
    ...
    if (editMode && postId) {
        ...
        document.getElementById('submit-btn').innerHTML = '<i class="fas fa-save"></i> 保存修改';
    }
...
function sharePost(postId) {
    ...
    const shareText = `${post.username} 的校园圈动态：${post.content.substring(0, 100)}...`;
    ...
    gallery.innerHTML = `
        ...
        <div class="gallery-content">
            <div class="gallery-header">
                <span class="gallery-counter">${startIndex + 1} / ${post.images.length}</span>
                <button class="gallery-close" onclick="closeImageGallery()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="gallery-main">
                <img src="${post.images[startIndex]}" alt="图片 ${startIndex + 1}" class="gallery-image">
